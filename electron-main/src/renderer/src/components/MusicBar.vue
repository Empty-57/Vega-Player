...
watch(currentIndex, () => {
  ...
  EventBus.emit('getMetadata', {path: playList.value[currentIndex.value].path, currentLocal: currentLocal.value});
})
...
EventBus.on('updatePlayList', ({path, title, artist, localName}) => {
  if (localName === currentLocal.value && currentLocal.value && playList.value.findIndex(i => i.path === path) === -1) {
...
EventBus.on('insert2next', ({path, title, artist, localName}) => {
  if (localName === currentLocal.value && currentLocal.value && playList.value.findIndex(i => i.path === path) !== -1) {
    playList.value.splice(playList.value.findIndex(i => i.path === path), 1);
...
<template>
  <div
    class="fixed w-screen h-16 dark:bg-neutral-900 bg-neutral-200 border-t-[0.1px] dark:border-neutral-700 border-gray-400 shadow-xl bottom-0 left-0 flex items-center justify-start px-3 **:select-none z-10"
  >
    ...
    <div class="flex h-10 flex-col items-center justify-start mx-3 w-1/3">
      ...
      <div class="flex items-center justify-center gap-x-4 mt-1 h-4">
        <div class="w-4">
          <svg v-if="playMode==='random'"
               class="fill-zinc-900 dark:fill-zinc-200 dark:hover:fill-cyan-600 hover:fill-red-500/80 cursor-pointer" height="13"
               viewBox="0 0 1024 1024" width="13" xmlns="http://www.w3.org/2000/svg" @click="setPlayMode">
            <path
          ...
          <svg v-if="playMode==='order'"
               class="fill-zinc-900 dark:fill-zinc-200 dark:hover:fill-cyan-600 hover:fill-red-500/80 cursor-pointer" height="14"
               viewBox="0 0 1024 1024" width="14" xmlns="http://www.w3.org/2000/svg" @click="setPlayMode">
            <path
        ...
        <div class="relative size-4 flex items-center justify-center">
          ...
          <svg class="fill-zinc-900 dark:fill-zinc-200 dark:hover:fill-cyan-600 hover:fill-red-500/80 cursor-pointer"
               height="13"
               viewBox="0 0 1024 1024" width="13" xmlns="http://www.w3.org/2000/svg"
               @click.stop="() => {volume_dropdown = !volume_dropdown}">
            <path
    ...
    <div class="w-1/3 h-10 flex items-center justify-end gap-x-4 mr-2">
      ...
      <svg class="fill-zinc-900 dark:fill-zinc-200 dark:hover:fill-cyan-600 hover:fill-red-500/80 cursor-pointer"
        ...
        <path
          d="M793.941 160H230.06C191.36 160 160 191.616 160 230.059v563.84C160 832.64 191.616 864 230.059 864h563.84c38.698 0 70.058-31.616 70.058-70.059V230.06C864 191.36 832.384 160 793.941 160z m5.718 640c0-0.299-575.659-0.341-575.659-0.341 0.299 0 0.341-575.659 0.341-575.659 0 0.299 575.659 0.341 575.659 0.341-0.299 0-0.341 575.659-0.341 575.659z"></path>
        <path
      ...
      <svg class="fill-zinc-900 dark:fill-zinc-200 dark:hover:fill-cyan-600 hover:fill-red-500/80 cursor-pointer"
           height="24" viewBox="0 0 1024 1024" width="24" xmlns="http://www.w3.org/2000/svg">
        <path
